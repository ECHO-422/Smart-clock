...
int Ble_ReceData(int *time)
{
    ...
    {
        len = USART3_RX_STA & 0X7FFF; //数据长度
        USART3_RX_BUF[len] = 0; //添加结束符
        USART3_RX_STA = 0;//没有接受到数据
       ...
       // if (strstr((const char *)USART3_RX_BUF, LED0_OFF) && (len == (sizeof(LED0_OFF) - 1))) LED0 = 1; /*LED0关闭*/
				if ((const char )USART3_RX_BUF[0]=='T'  )  //设置时间
				{
						printf("\r\n设置时间\r\n");
						for(i=1;i<len;i++)
						{
							if((const char )USART3_RX_BUF[i] == '/' || (const char )USART3_RX_BUF[i] == ':' ||(const char )USART3_RX_BUF[i] == ' ')
							{
									time[j++]=num;
									num=0;
							}
				...
				else if ((const char )USART3_RX_BUF[0]=='A'  )  //设置时间
				{
						printf("\r\n设置闹钟时间\r\n");
						for(i=1;i<len;i++)
						{
